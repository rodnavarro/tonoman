...
your own connected accounts, over your own data. Celine talks to Nelly-as-Celine; you talk to
Nelly-as-you. A single brain keyed by the *agent* would pool everyone's meetings into one store, so
anyone talking to Nelly could read Celine's — the exact failure the per-user model exists to prevent.

Keying the brain by **account** closes that loop. The Plaud Talent, polling Celine's per-person Plaud,
files her recap in **Celine's** brain. When you open the same agent, you read **yours**. The Users tab
