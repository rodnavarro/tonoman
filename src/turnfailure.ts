 ...
 *  registry. That remains the right source: a FACT about the agent, never a file check — atlas
...
export function isNotLoggedInError(msg: string): boolean {
  return /not logged ?in|please run \/login|no credentials found|invalid api key|oauth (token|session) (has )?expired|could not be refreshed|authentication_error|401 unauthorized|missing bearer or basic authentication/i.test(
    msg || "",
  );
}
