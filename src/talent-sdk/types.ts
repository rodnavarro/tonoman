...
export type CapabilityName = 'transcribe' | 'infer' | 'publish' | 'calendar' | 'page' | 'lorealistar';
...
export interface TalentOutcome {
  ...
  steer?: string;
  /** Words to be said EXACTLY as written, to the person the run is for — a source's own figures: how
   *  many are left, when it ends, the link. The runtime posts them as they are; the agent does not put
   *  them in its own words, because a number reworded is a number that can come out wrong
   *  (TALENT-FIGURES-SAID-AS-GIVEN). Use `steer` for everything else. */
  say?: string;
...
export interface TalentContext {
  ...
  cap: {
    ...
    /** File ONE page the Talent wrote, at a path it names, into the brain the run is pointed at
     *  (TALENT-FILES-A-PAGE). Rejects when there is no brain to file into, or the run may not write it. */
    page(p: { path: string; content: string; note?: string }): Promise<{ filed: boolean; path: string; brain: string }>;
    /** What LOREALISTAR said about one drop, for the person this run is for. The runtime holds their
     *  login and does the looking; the Talent never sees either (DROPS-LOGIN-SEALED). `drop` is absent
     *  when the runtime no longer has it. */
    lorealistarDrop(i: { id: string }): Promise<{ drop?: Record<string, unknown> & { id: string; name: string; type: string; status: string }; seen?: string }>;
  };
