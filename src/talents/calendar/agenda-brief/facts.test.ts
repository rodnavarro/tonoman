...
function ev(summary: string, from: string, to: string): CalendarEvent {
  return { summary, start: nyc(from), end: nyc(to), attendees: [], source: { kind: 'ics', alias: 'foley' } };
}
...
describe('agendaFacts', () => {
  ...
  it('finds back-to-back runs of three with small gaps', () => {
    const f = agendaFacts(day, nyc('07:00'), TZ);
    expect(f.backToBack.map((r) => r.map((e) => e.summary))).toEqual([['Lunch & learn', '1:1 Celine', 'Axiplex sync']]);
  });
  ...
  it('at midday counts only what is not over, and frees from now', () => {
    ...
    expect(f.remaining.map((e) => e.summary)).toEqual(['Lunch & learn', '1:1 Celine', 'Axiplex sync']);
