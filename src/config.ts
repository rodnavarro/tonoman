...
export interface AgentConfig {
   ...
   *  tenant (`acme-sapien`) to key one worker's many agents. Renaming an agent changes this, and
   *  the worker carries it into the agent's context so a rename reaches how it introduces itself. */
  displayName?: string;
  /** The tenant slug this agent belongs to (`acme`, `initech`). Carried so the worker can render
  ...
  /** How much shell this agent's turns have (CLI-SHELL-SETTING in Tonoman Cloud): `full` for an agent
   * that codes; anything else, undefined included, is `tonoman` only. */
  shell?: "tonoman" | "full";
