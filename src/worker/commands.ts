...
export interface CommandDeps {
  ...
  /** The 5h/7d allowance — the SPEAKER's own login on an agent that runs inference per person. */
  windows(agent: string, user?: string): Promise<UsageWindow[]>;
 ...
 *  test failing. */
export const CONNECT_KINDS = ["claude", "codex", "plaud", "google", "outlook", "ics", "lorealistar"] as const;
...
export async function run(
  ...
  switch (cmd.name) {
    ...
    case "logout": {
      ...
      if (which === "lorealistar") {
        if (!deps.disconnectLorealistar) return "I can't watch LOREALISTAR on this deployment.";
        return deps.disconnectLorealistar(agent, user);
      }
      if (which !== "plaud") return unknownConnector(which, ["plaud", "lorealistar", ...INFERENCE_KINDS]);
    ...
    case "connect": {
      ...
      // An email and a PASSWORD: a private dialog, never a typed command (DROPS-OWN-LOGIN).
      if (which === "lorealistar") {
        if (!deps.connectLorealistar) return "I can't watch LOREALISTAR on this deployment.";
        return deps.connectLorealistar(agent, conversation);
      }
    ...
    case "status":
    case "usage": {
      const windows = await deps.windows(agent, user).catch(() => [] as UsageWindow[]);
      const account = (await deps.claudeAccount?.(agent, user).catch(() => "")) ?? "";
      const provider = deps.inferenceProvider?.(agent) ?? "Claude";
    ...
    case "connections": {
      ...
      const lines = list.map((c) => {
        ...
        // it. The kind is a detail; `Globex Outlook ICS` means something, `ics` does not.
