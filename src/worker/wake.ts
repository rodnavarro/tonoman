...
export interface WakeRequest {
  /** Roster name of the agent to speak, e.g. "initech-nelly". */
  agent: string;
  ...
  text: string;
  /** Files to go with the text, as paths on the worker — what a person's attachments become. Lets a
   *  test drive a turn with a photo without Slack. The endpoint is token-guarded and local. */
  files?: string[];
...
export interface WakeDeps {
   ...
   *  addressed to them. */
  ask(agent: string, conversation: string, text: string, user?: string, files?: string[]): Promise<void>;
...
export function parseWake(body: unknown): { ok: true; req: WakeRequest } | { ok: false; error: string } {
  ...
  const files = (b as { files?: unknown }).files;
  if (files !== undefined && !(Array.isArray(files) && files.every((f) => typeof f === "string" && f))) return { ok: false, error: "files must be a list of paths" };
  return {
    ok: true,
    req: { agent: b.agent, user: b.user, conversation: b.conversation, text: b.text, verbatim: b.verbatim === true, files: files as string[] | undefined },
  };
}
...
export function serveWake(o: WakeServerOptions, signal: AbortSignal): boolean {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      const { agent, user, conversation, text, verbatim, files } = v.req;
      ...
      void (async () => {
        try {
          ...
          else await o.deps.ask(agent, conv, text, user, files);
        } catch (e) {
