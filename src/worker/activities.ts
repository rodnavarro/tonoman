...
import { CLI_NOTE } from "../brains/cli";
import { ownName } from "./sentfiles";
...
import { authFailureState, notLoggedInNotice, isNotLoggedInError } from "../turnfailure";
...
export interface TurnDeps {
   ...
   *  the agent could not: no such channel, or it has not been invited. Never a person — `say` is. */
  sayIn?(agent: string, channel: string, text: string): Promise<boolean>;
  ...
  brains?: TurnBrains;
  /** The Linux user a piece of this agent's work runs as, made and proven (turn-user.md); absent on
   *  a worker whose turns run as itself. Throws rather than fall back to the worker's own user. */
  turnUser?: (agent: string, user: string | undefined) => Promise<{ uid: number; home: string }>;
  ...
  lorealistar?: {
    ...
    /** Everyone who has connected their own login for this agent. */
    users(agent: string): Promise<string[]>;
    /** One look for one person: what is new, and anything they should hear about the watch itself. */
    lookFor(agent: string, user: string, timezone?: string): Promise<{ news: { id: string; name: string }[]; notice?: string; alive?: string }>;
  ...
  timezoneOf?(agent: string): string | undefined;
  /** Where turns' folders are made. A folder only the worker can write, when turns run as their own
   *  users: a turn's folder becomes its user's, and in a shared temp folder its owner could rename
   *  it away and leave a link in its place. Absent = the system's temp folder, as always. */
  turnsRoot?: string;
...
export interface VoiceConfig {
   ...
   *  outside whether a recap went to Rod or to Priya.
   *
...
export interface TurnBrains {
  ...
  /** What the person sent, as it arrived, under the names the agent sees (RECEIPT-FILES-WHAT-WAS-SENT). */
...
/** Left on a turn the worker's own stopping cut short (CONVO-RESTART-SAYS-SO). */
export const RESTART_NOTE = "_— cut short by a restart on my side, not by you. Please send your message again._";
...
export function makeActivities(deps: TurnDeps) {
  ...
  return {
    ...
    async runTalent(input: {
      ...
      try {
        ...
        if (outcome.status === "done" && outcome.say) {
          if (input.channel) {
            const posted = (await deps.sayIn?.(input.agent, input.channel, outcome.say).catch(() => false)) ?? false;
        ...
        return {
          ...
          summary: (outcome.steer ?? outcome.say ?? outcome.summary ?? "").slice(0, 2000) || undefined,
          links: outcome.links,
        };
      } finally {
        clearInterval(beat);
    ...
    async tellPrivately(input: { agent: string; user: string; text: string }): Promise<void> {
      await (deps.dm ?? deps.say)?.(input.agent, input.user, input.text);
    },
...
async function oneTurn(deps: TurnDeps, input: TurnInput): Promise<void> {
    ...
    if (brains && input.user) {
      ...
      // A finance document is private from the moment it arrives, not from the moment a brain is
      // used (RECEIPT-PRIVATE-THROUGHOUT): the agent may ask about the amount before it files
      // anything. The thread remembers it drew on the finances brain, so the person's answer to
      // that question, sent later with no file, is private too.
      if (receipts && (input.mediaPaths ?? []).some(isFinanceDocument)) {
        finance = true;
        const reach = await brains.reach(input.agent, input.user).catch(() => null);
        if (reach?.has(receipts.brainId)) {
          await brains.remember(input.agent, key, [receipts.brainId]).catch(() => {});
          prior = [...new Set([...prior, receipts.brainId])];
        }
      }
    ...
    let msgId = "";
    /** What is on screen now — recorded only once Slack has taken it. */
    let shown = "";
    /** ONE writer of the answer's message at a time. The words are pushed from two places — as they
     *  arrive, and from the timer when no more arrive (CONVO-WORDS-DO-NOT-STALL) — and unserialised,
     *  two of them could both see no message yet and post the answer twice, or land out of order and
     *  put older words over newer ones. Everything that writes the answer waits on this. */
    let rendering: Promise<void> = Promise.resolve();
    const show = (text: string): Promise<void> =>
      (rendering = rendering
        .catch(() => {})
        .then(async () => {
          if (done || text === shown) return;
          if (!msgId) msgId = await reply.send(text);
          else await reply.update(msgId, text);
          shown = text;
        }));
    /** Words that have been said and are not on screen yet, pushed by the TIMER. The stream only
     *  pushes when a NEWER word arrives, so a sentence said just before the tools run sat frozen at
     *  its first word for as long as they ran ("I", for minutes, in production). Never for a held
     *  turn: nothing of its answer shows until delivery has been decided. */
    const flushWords = (): void => {
      if (held || done || ctx.cancellationSignal.aborted) return;
      if (!answer.trim() || answer === shown) return;
      const now = Date.now();
      if (now - lastEdit < EDIT_INTERVAL_MS) return;
      lastEdit = now;
      void show(answer).catch(() => {});
    };
    ...
    let posting = false;
    let lastTick = 0;
    /** When the agent last said a word — what tells "answering" from "said something, now working". */
    let lastWordAt = 0;
    const tick = async (first = false): Promise<void> => {
      ...
      const now = Date.now();
      // …but only while words are actually ARRIVING. An agent that said a sentence and went to its
      // tools is not editing its message any more, and a clock that then moved every five seconds
      // read as stuck for exactly the part of the turn that takes longest.
      const answering = !!answer && now - lastWordAt < 2 * EDIT_INTERVAL_MS;
      if (!first && now - lastTick < (answering ? ANSWERING_TICK_MS : EDIT_INTERVAL_MS)) return;
      lastTick = now;
      posting = true;
      try {
    ...
    /** The note, put up BEFORE the agent's first words (CONVO-WORK-LOG-WHEN-IT-SPEAKS-FIRST). An
     *  agent that spoke inside the first tick never got a note or a clock at all, because one made
     *  after the words would sit under them. Once only: a note Slack refused is not asked for again
     *  on every word. A tick already posting it is waited for, never doubled (see `posting`). */
    let noteTried = false;
    const noteBeforeWords = async (): Promise<void> => {
      if (noteId || noteTried) return;
      noteTried = true;
      await inflight.catch(() => {});
      if (!noteId) await (inflight = tick(true)).catch(() => {});
    };
    ...
    ctx.cancelled.catch(async (why: unknown) => {
      if (done) return;
      dropLog();
      void reply.settle?.().catch(() => {});
      // WHY it stopped decides what is said. A worker that is stopping cancels its turns too, and
      // there is no new message then: saying one was being worked on left a person waiting for an
      // answer that nobody was writing (CONVO-RESTART-SAYS-SO). Temporal names the reason.
      const stopping = (why as Error | undefined)?.message === "WORKER_SHUTDOWN";
      // Whatever was being written lands first, so this is the last word on the message.
      await rendering.catch(() => {});
      const said = held ? "" : answer.trim();
      if (stopping) {
        // Said even when nothing was on screen yet: the person asked, and nothing else will tell them.
        const note = `${said ? `${said}\n\n` : ""}${RESTART_NOTE}`;
        await (msgId ? reply.finalize(msgId, note) : reply.send(note)).catch(() => {});
        return;
      }
      ...
      if (!msgId) return;
      await reply.finalize(msgId, `${said}\n\n_— interrupted; working on your new message_`).catch(() => {});
    });

    // What the agent can read, and who is asking. Without the first the second brain is present
    // on disk but the model has no reason to look at it; without the second "Hi Priya" is a
    ...
    const identity = found.cfg.system_prompt_file
      ...
      : "";
    let turnSystemFile = path.join(
      os.tmpdir(),
      `tonoman-turn-${input.agent.replace(/[^A-Za-z0-9_-]/g, "")}-${Date.now()}-${Math.random().toString(36).slice(2)}.md`,
    );
    // Private from its first byte, and never over a file that is already there: /tmp is everyone's.
    ...
    const turnDir = await fs.mkdtemp(path.join(deps.turnsRoot ?? os.tmpdir(), "tonoman-turn-"));
    if (brainTurn) brains?.bind?.(brainTurn.token, turnDir);
    // Into the turn's own folder, so it is handed to the turn's user with everything else in it and
    // is never a file of the worker's sitting in a folder everyone can see.
    const promptInside = path.join(turnDir, ".tonoman-system.md");
    await fs.rename(turnSystemFile, promptInside).then(() => (turnSystemFile = promptInside), () => {});
    ...
    const media: string[] = [];
    let unplaced = 0;
    // Each file is read ONCE, from where the worker keeps it: that is what the broker is handed, as it
    // arrived, before the agent runs (RECEIPT-FILES-WHAT-WAS-SENT), and what is copied for the agent to
    // look at. Two files with the same name each get a name of their own (CONVO-ATTACHED-FILES).
    const arrived: { name: string; bytes: Buffer }[] = [];
    for (const m of input.mediaPaths ?? []) {
      const bytes = await fs.readFile(m).catch(() => null);
      if (bytes) arrived.push({ name: path.basename(m), bytes });
      else unplaced++;
    }
    // A receipt is rarely one message: the photo, the agent's question, then an answer with no photo
    // on it — and the filing happens there. So what this person sent earlier in this conversation is
    // still the broker's to file, and still in the folder to look at (CONVO-SENT-FILES-STAY). Kept
    // first, then read back: that is what gives every file, old and new, one name of its own.
    // A name the worker uses in this folder itself is not a person's to send: a file called what the
    // prompt is called would otherwise be written over it.
    for (const f of arrived) if (f.name.toLowerCase().startsWith(".tonoman-")) f.name = `sent-${f.name.replace(/^[.]+/, "")}`;
    let sent = arrived.map((f, i) => ({ ...f, name: ownName(f.name, new Set(arrived.slice(0, i).map((x) => x.name))) }));
    let earlier: string[] = [];
    if (deps.sentFiles && input.user) {
      const before = new Set((await deps.sentFiles.recent(input.agent, input.conversation, input.user).catch(() => [])).map((f) => f.name));
      await deps.sentFiles.keep(input.agent, input.conversation, input.user, arrived).catch(() => {});
      const all = await deps.sentFiles.recent(input.agent, input.conversation, input.user).catch(() => []);
      if (all.length) {
        sent = all;
        earlier = all.filter((f) => before.has(f.name)).map((f) => f.name);
      }
    }
    for (const f of sent) {
      const dest = path.join(turnDir, f.name);
      // `wx`: never over a file that is already there — the prompt's, or another attachment's.
      const placed = f.name.toLowerCase().startsWith(".tonoman-") ? false : await fs.writeFile(dest, f.bytes, { flag: "wx" }).then(() => true, () => false);
      // Only what arrived with THIS message is shown to the agent as new.
      if (placed && !earlier.includes(f.name)) media.push(dest);
      else if (!placed && !earlier.includes(f.name)) unplaced++;
    }
    if (brainTurn && sent.length) brains?.attach?.(brainTurn.token, sent);
    const mediaNote =
      (media.length ? `\n\nAttached file(s) — read them:\n${media.map((p) => `- ${p}`).join("\n")}` : "") +
      (earlier.length ? `\n\nSent earlier in this conversation — still in your folder, and still fileable by name:\n${earlier.map((n) => `- ${n}`).join("\n")}` : "") +
    ...
    try {
      try {
        ...
        // A turn that could not sign in also fails at startup, before a token — and is not a lost
        // memory: the conversation is all still there, and the person only has to sign in. Starting
        // afresh there threw a thread's memory away for nothing (it did, Sep 19).
        const signedOut = isNotLoggedInError(String((e as Error)?.message ?? e));
        if (!session || produced || signedOut || ctx.cancellationSignal.aborted) throw e;
        console.error(
      ...
      clearInterval(ticker);
      // A folder that became a turn user's is deleted AS that user: the worker walking it as root
      // could be led, by a link put there, to delete something of its own. What is left for the worker
      // is the empty folder — and only that, never what is in it.
      if (await deps.dropTurnDir?.(turnDir).catch(() => true)) await fs.rmdir(turnDir).catch(() => {});
      else {
    ...
    // A write of the words may still be in flight — and if it is the FIRST, the message has no id
    // yet: the final below would post a second copy of the answer instead of finishing that one.
    await rendering.catch(() => {});
    ...
    const footer = await deps.footer?.(input.agent, input.conversation, usage, input.user).catch(() => null);
    ...
    if (brains && input.user && (drawnOn.length || finance) && audience.kind !== "self") {
      audience = await brains.audience(input.agent, input.conversation, input.user).catch((e: Error) => ({ kind: "unknown", why: e.message }) as Audience);
      const now = await brains.reach(input.agent, input.user).catch(() => null);
      const readable = audience.kind === "members" ? await brains.readableByAll(input.agent, drawnOn, audience.members).catch(() => null) : null;
      // A finance document never goes to a channel, whoever there can read the finances brain.
      if (audience.kind !== "self" && (finance || decideRoute(audience, drawnOn, readable).to === "private")) {
        // Why it went private, by kind and reason only — never the answer or a brain's content.
        console.log(`worker: ${input.agent} answer sent privately — ${finance ? "a finance document" : `audience ${audience.kind}`}${"why" in audience ? `: ${audience.why}` : ""}`);
        const names = drawnOn.map((b) => now?.get(b)).filter((n): n is string => !!n);
        const sent = await brains.dm(input.agent, input.user, `${finance ? FINANCE_REASON : privateReason(names, audience)}\n\n${final}`).catch(() => false);
...
/** PURE: a file that may be a receipt, invoice or statement (RECEIPT-DOCUMENTS). */
export function isFinanceDocument(file: string): boolean {
...
export function receiptsOf(talents: { name: string; version?: number; config?: Record<string, unknown> }[] | undefined): { brainId: string; entities?: string[]; version?: number } | undefined {
  ...
  const entities = (Array.isArray(raw) ? raw.map(String) : typeof raw === "string" ? raw.split(",") : []).map((e) => e.trim()).filter(Boolean);
...
function guidanceFor(provider: "claude" | "codex", receipts?: { entities?: string[] }): string {
  ...
  return [
    ...
    "- Read only what the question needs. Name the brain and page each fact came from.",
...
const RECEIPTS_NOTE = [
  ...
  "One document with several payments, or a statement whose payments may already have been sent as receipts, is NOT filed until the person says which rows it stands for: ask first, so nothing is counted twice.",
  "To add a note to a receipt ALREADY filed, or put right what it was for, edit the brain: find its row in the year's ledger with `tonoman brain search`, then `tonoman brain edit` that row — its note only, never its amount, vendor or file. Do not rewrite the ledger with `brain write`: it is too long to read whole.",
  "More photos of a payment already filed go with `--extra`. A refund, a zero or a foreign amount is asked about, not filed.",
  "For a business, the note says the expense is ordinary and necessary. Report only what the command returned — never an amount, a path or 'filed' of your own. `tonoman receipts --help` has the details.",
].join("\n");
