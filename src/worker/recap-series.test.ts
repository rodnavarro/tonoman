...
const journal = { path: "Meeting-Journals", fallback: "unclassified", routes: [{ id: "globex-meetings", when: "Globex" }] };
const rec: Recording = {
...
describe("pathsFor with a matched meeting", () => {
  it("files under the series folder and names the file after the meeting", () => {
    const where = pathsFor(journal, rec, "globex-meetings", recapSlugHint({ meeting: "API Team Standup", highlights: ["x"], summary: "s" }), undefined, "API Team Standup");
    expect(where.page).toBe("Meeting-Journals/globex-meetings/API-Team-Standup/2026-09-17-1400-api-team-standup.md");
    expect(where.folder).toBe("Meeting-Journals/globex-meetings/API-Team-Standup/2026-09-17-1400-api-team-standup");
  });
  it("keeps today's path when nothing matched", () => {
    expect(pathsFor(journal, rec, "globex-meetings", "budget review").page).toBe(
      "Meeting-Journals/globex-meetings/2026-09-17-1400-budget-review.md",
    );
  });
});
...
describe("a recap filed under its series still counts as published", () => {
  it("is not transcribed again on the next poll", async () => {
    ...
    const page = path.join(dir, "Meeting-Journals/globex-meetings/API-Team-Standup/2026-09-17-1400-api-team-standup.md");
...
describe("calendarSection times", () => {
  it("prints candidate times in the tenant's timezone, not UTC", () => {
    const md = calendarSection(
      ...
      [{ summary: "API Team Standup", start: Date.parse("2026-09-17T14:00:00Z"), end: Date.parse("2026-09-17T14:30:00Z"), attendees: [], attendeeEmails: [], uid: "", location: "", source: { kind: "ics", alias: "globex" } }],
      "America/New_York",
    );
...
describe("calendar.route.<alias> — the calendar decides the route", () => {
  it("is read from flow properties by alias", () => {
    const s = voiceSettings({ "calendar.route.globex": "globex-meetings", "calendar.route.personal": " acme-meetings ", "calendar.route.": "x" }, {});
    expect(s.calendarRoutes).toEqual({ globex: "globex-meetings", personal: "acme-meetings" });
  });
});
