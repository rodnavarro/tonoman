...
describe("the volume", () => {
  ...
  it("keeps a member's own account out of the shared one and out of another member's", async () => {
    ...
    const root = await tmp();
    const s = fileStore(root);
    await s.save("initech", { access_token: "shared" });
    await s.save("initech", { access_token: "priya" }, "U_PRIYA");
    await s.save("initech", { access_token: "rod" }, "U_ROD");
    expect((await s.load("initech"))?.access_token).toBe("shared");
    expect((await s.load("initech", "U_PRIYA"))?.access_token).toBe("priya");
    expect((await s.load("initech", "U_ROD"))?.access_token).toBe("rod");
    // Clearing one member touches neither the shared account nor the other member.
    await s.clear("initech", "U_PRIYA");
    expect(await s.load("initech", "U_PRIYA")).toBeUndefined();
    expect((await s.load("initech"))?.access_token).toBe("shared");
    expect((await s.load("initech", "U_ROD"))?.access_token).toBe("rod");
  });
  ...
  it("listUsers returns the members who connected, and NEVER the shared account", async () => {
    const root = await tmp();
    const s = fileStore(root);
    await s.save("initech", { access_token: "shared" }); // the shared account has no member
    await s.save("initech", { access_token: "a" }, "U0AAAAAAA1");
    await s.save("initech", { access_token: "b" }, "U0RODNAV99");
    const users = (await s.listUsers("initech")).map((u) => u.user).sort();
    expect(users).toEqual(["U0AAAAAAA1", "U0RODNAV99"]);
    // Each carries a connect time (the floor), and an agent with no members lists nobody.
    expect((await s.listUsers("initech"))[0]?.connectedAt).toBeTypeOf("number");
...
describe("Tonoman Cloud", () => {
  ...
  it("scopes a member's own secret in the REF, leaving the shared ref untouched", async () => {
    ...
    let url = "";
    const s = deps((async (u: string) => ((url = u), ok({ value: "{}" }))) as unknown as typeof fetch);
    await s.load("initech", "U_PRIYA");
    expect(url).toBe(`http://api/v1/system/agents/guid-1/secrets/${encodeURIComponent("plaud.tokens:U_PRIYA")}`);
    await s.load("initech");
    expect(url).toBe("http://api/v1/system/agents/guid-1/secrets/plaud.tokens");
  });
  ...
  it("listUsers reads members out of the secrets list, excluding the shared ref and non-Plaud refs", async () => {
      ...
      if (u.endsWith("/secrets")) {
        return ok({
          secrets: [
            ...
            { ref: "plaud.tokens:U0AAAAAAA1", createdAt: "2026-09-10T01:00:00Z" },
    ...
    const users = await s.listUsers("initech");
    expect(users.map((u) => u.user).sort()).toEqual(["U0AAAAAAA1", "U0RODNAV99"]);
    expect(users.find((u) => u.user === "U0AAAAAAA1")?.connectedAt).toBe(Date.parse("2026-09-10T01:00:00Z"));
  });
  ...
  it("puts the ref in the path safely", async () => {
    ...
    const s = cloudStore({
      ...
      token: "t",
      guidOf: () => "g",
      ref: "claude.credentials:initech-nelly",
      fetchImpl: (async (u: string) => ((url = u), ok({ value: "{}" }))) as unknown as typeof fetch,
    });
    await s.load("nelly");
    expect(url).toContain("claude.credentials%3Ainitech-nelly");
  });
});
...
describe("plaudRefFor / userFromPlaudRef — scope lives in the ref", () => {
  it("appends the user to the base ref, and leaves the shared ref bare", () => {
    expect(plaudRefFor("plaud.tokens", "U_PRIYA")).toBe("plaud.tokens:U_PRIYA");
  ...
  it("round-trips a Slack id back out of a scoped ref, and yields undefined otherwise", () => {
    expect(userFromPlaudRef("plaud.tokens:U0AAAAAAA1")).toBe("U0AAAAAAA1");
    expect(userFromPlaudRef("plaud.tokens")).toBeUndefined();
    expect(userFromPlaudRef(undefined)).toBeUndefined();
    expect(userFromPlaudRef("something.else:U0AAAAAAA1")).toBeUndefined();
