...
describe("voiceSettings — routing is N rows", () => {
  const props = {
    ...
    "route.globex-meetings": "Globex — the employer",
    "route.acme-meetings": "Acme — Rod's own company",
  };
  ...
  it("reads one route per row, and the id IS the folder", () => {
    ...
    expect(j.routes.map((r) => r.id)).toEqual(["acme-meetings", "globex-meetings"]);
  });
...
describe("describe", () => {
  it("says where recaps go and how they are filed, because silence looks like success", () => {
    const line = describeVoice(
      voiceSettings(
        {
          credential_ref: "plaud-initech:PLAUD_TOKEN",
          notify_channel: "C1",
          "journal.path": "MJ",
          "journal.fallback": "unclassified",
          "route.a": "A",
  ...
  it("names the DM recipient when there is no channel", () => {
    expect(
      describeVoice(voiceSettings({ credential_ref: "plaud-initech:PLAUD_TOKEN", notify_user: "U1" }, {})),
...
describe("voiceSettings — the off switch", () => {
  ...
  it("says so loudly in the boot line — a silent flow looks identical to a working one", () => {
    expect(
      describeVoice(voiceSettings({ credential_ref: "plaud-initech:PLAUD_TOKEN", enabled: "false" }, {})),
    ).toContain("DISABLED");
  });
...
describe("voiceSettings — whose Plaud account", () => {
  it("takes the credential from the tenant's own row", () => {
    expect(voiceSettings({ credential_ref: "plaud-initech:PLAUD_TOKEN" }, {}).credentialRef).toBe(
      "plaud-initech:PLAUD_TOKEN",
    );
  });
  ...
  it("says it is waiting for a login, which is a state and not a fault", () => {
    ...
    // that way — "waiting for Priya", not "broken".
