...
describe("unprefixSlash — /<app-name>-<command> routes to the command", () => {
  it("takes the app's name off", () => {
    ...
    expect(parse(unprefixSlash("!nelly-initech-s-model opus"))).toEqual({ name: "model", arg: "opus" });
  });
...
describe("run", () => {
  ...
  it("lists connections by the name a person gave them, not by kind", async () => {
    // `Globex Outlook ICS` means something to somebody. `ics` does not.
    const d = {
      ...deps(),
      connections: async () => [
        { kind: "ics", alias: "globex", label: "Globex Outlook ICS", status: "connected" },
        { kind: "google", alias: "personal", status: "expired", externalAccount: "rod@example.com" },
      ],
    };
    const out = (await run(d, "sapien", "c", { name: "connections", arg: "" })) ?? "";
    expect(out).toContain("Globex Outlook ICS");
    expect(out).toContain("rod@example.com");
  ...
  it("scopes the change to the conversation it was typed in", async () => {
    const setModel = vi.fn();
    const d = deps({ setModel });
    await run(d, "nelly", "priya-thread", { name: "model", arg: "opus" });
    expect(setModel).toHaveBeenCalledWith("nelly", "priya-thread", "opus");
...
describe("every connector the agent offers is one it can actually run", () => {
  ...
  it("DROPS-OWN-LOGIN `!connect lorealistar` opens the private dialog for it, and asks for nothing in the channel; `!disconnect lorealistar` forgets the SPEAKER's login", async () => {
    const calls: string[] = [];
    const d = {
      ...deps,
      connectLorealistar: async (agent: string, conversation: string) => (calls.push(`connect ${agent} ${conversation}`), ""),
      disconnectLorealistar: async (agent: string, user?: string) => (calls.push(`disconnect ${agent} ${user}`), "forgotten"),
    } as never;
    // Anything typed after the name — an email, a password — is not taken: only the dialog takes them.
    expect(await run(d, "mia", "T1/C1", { name: "connect", arg: "lorealistar test+stef@tonoman.com pw" }, "USTEF")).toBe("");
    expect(await run(d, "mia", "T1/C1", { name: "disconnect", arg: "lorealistar" }, "USTEF")).toBe("forgotten");
    expect(calls).toEqual(["connect mia T1/C1", "disconnect mia USTEF"]);
    expect(await run(deps, "mia", "T1/C1", { name: "connect", arg: "lorealistar" })).toMatch(/can't watch LOREALISTAR/);
  });
...
describe("!connections answers from every store, not just the registry", () => {
  ...
  it("lists the Claude subscription, which an agent is plainly connected to", async () => {
    const deps = {
      ...base,
      connections: async () => [],
      claudeAccount: async () => "rod@example.com (max)",
    } as unknown as Parameters<typeof run>[0];
    const out = await run(deps, "a", "c", { name: "connections", arg: "" });
    expect(out).toContain("Claude subscription");
    expect(out).toContain("rod@example.com (max)");
  });
  ...
  it("shows a calendar and a legacy credential together", async () => {
    const deps = {
      ...base,
      connections: async () => [{ kind: "ics", alias: "globex", label: "Globex Outlook ICS", status: "connected" }],
      plaudConnected: async () => true,
    } as unknown as Parameters<typeof run>[0];
    const out = await run(deps, "a", "c", { name: "connections", arg: "" });
    expect(out).toContain("Globex Outlook ICS");
...
describe("!status and !connections name the agent's own provider", () => {
  ...
  it("a codex agent's !status says Codex, not Claude", async () => {
    const d = {
      ...base,
      claudeAccount: async () => "rod@example.com (plus)",
      inferenceProvider: () => "Codex",
    } as unknown as CommandDeps;
    const out = (await run(d, "a", "c", { name: "status", arg: "" })) ?? "";
    expect(out).toContain("Codex account: rod@example.com (plus)");
  ...
  it("!connections lists the subscription under the provider that actually answers", async () => {
    const d = {
      ...base,
      claudeAccount: async () => "rod@example.com",
      inferenceProvider: () => "Codex",
    } as unknown as CommandDeps;
