...
//
// Both were written after the same bug. Sapien answered a question about a Globex meeting, was
...
describe("sessionStore — a thread continues one session", () => {
  ...
  it("NEVER lets one agent resume another's conversation", async () => {
    ...
    // else. This worker serves Initech and Acme, in two different workspaces. Keyed by the
  ...
  it("survives a conversation key with slashes in it, which every Slack key has", async () => {
    ...
    const s = store();
    const key = "T0AAAAAAA1/D0AAAAAAA1/1788771650.197619";
