...
describe("overviewMarkdown", () => {
  ...
  it("stamps the owner in frontmatter when a per-person account is named", () => {
    const md = overviewMarkdown(rec, { summary: "s", highlights: [], decisions: [], followups: [] }, undefined, [], [], "UTC", "U0AAAAAAA3");
    expect(md).toContain("\nowner: U0AAAAAAA3\n");
  });
...
describe("naming a recording", () => {
  ...
  it("falls back to what the meeting was about when the title is a clock", () => {
    ...
    const j = { path: "MJ", fallback: "unclassified", routes: [{ id: "acme", when: "..." }] };
    const p = pathsFor(j, rec, "acme", "Reviewed the Tonoman Cloud demo plan");
    expect(p.page).toBe("MJ/acme/2026-09-07-0310-reviewed-the-tonoman-cloud-demo-plan.md");
  });
...
describe("calendarSection — what it chose between, not just what it chose", () => {
  ...
  it("names which calendar each candidate came from", () => {
    expect(calendarSection(recap, CANDIDATES)).toContain("ics/globex");
  });
...
describe("pathsFor — a matched meeting names the file", () => {
  it("uses the meeting name when Plaud only gave a timestamp", () => {
    ...
    const rec = { id: "1", title: "2026-09-06 23:10:46", startTime: 0, duration: 0, stamp: "2026-09-07-0310" };
    const j = { path: "MJ", fallback: "unclassified", routes: [] };
    expect(pathsFor(j, rec, "globex", "API Team Standup").page).toBe(
      "MJ/globex/2026-09-07-0310-api-team-standup.md",
    );
  });
});
