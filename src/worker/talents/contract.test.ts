...
describe("the Receipts Talent (receipt.md in Tonoman Cloud)", () => {
  it("TALENT-BRAIN-SETTING Receipts is in the catalogue with a required brain setting, the tenant's entities, and no schedule", async () => {
    const { getTalent } = await import("./registry");
    const r = getTalent("receipts")!;
    expect(r).toBeDefined();
    expect(r.configSchema).toContainEqual(expect.objectContaining({ key: "brain", type: "brain", required: true }));
    expect(r.configSchema).toContainEqual(expect.objectContaining({ key: "entities", type: "text" }));
