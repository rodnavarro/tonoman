...
export async function sync(sources: SecondbrainSource[], o: SyncOptions): Promise<{ dir: string; label: string }[]> {
  ...
  for (const s of sources) {
    ...
    if (exists) {
      ...
      const was = (await run("git", ["rev-parse", "HEAD"], dir)).out.trim();
      r = await run("git", ["fetch", "--depth", "1", "origin", branch], dir);
      if (r.code === 0) r = await run("git", ["reset", "--hard", `origin/${branch}`], dir);
      // WHAT CAME DOWN TAKES THE PLACE OF WHAT WAS THERE (BRAIN-TODAY-CHECKOUT-STAYS-SMALL). A tip
      // fetched at depth 1 has no ancestor here to be a delta against, so the whole tree arrives
      // again, and the last one is never removed: git tidies on its own at fifty packs, and one
      // checkout of a 1.9 GB tree reached 57 GB in 33 packs first and took production's node out of
      // disk.
      //
      // THIS CHECKOUT HAS A SECOND WRITER. The recap publisher adds, commits and pushes in this same
      // folder (worker.ts builds its brainDir from the same path), under no lock shared with this.
      // Between its `git add` and its `git commit` its objects are referenced by nothing, which is
      // exactly what `--prune=now` takes - so nothing younger than an hour is pruned, and the reflog
      // keeps an hour too. That is git's own answer to a repository other things write to.
      //
      // Tidied when the tip moved, AND whenever packs have piled up anyway: a tidy that failed - out
      // of memory, out of disk - is tried again on the next refresh, not left until the tip moves.
      // One thread and a small window: this runs beside inference on a node short of memory.
      if (r.code === 0) {
        const is = (await run("git", ["rev-parse", "HEAD"], dir)).out.trim();
        const packs = await fs.readdir(path.join(dir, ".git", "objects", "pack")).then((n) => n.filter((f) => f.endsWith(".pack")).length, () => 0);
        if (is !== was || packs > 2) {
          await run("git", ["reflog", "expire", "--expire=1.hour.ago", "--expire-unreachable=1.hour.ago", "--all"], dir);
          const tidied = await run("git", ["-c", "pack.threads=1", "-c", "pack.windowMemory=128m", "-c", "pack.deltaCacheSize=64m", "gc", "--prune=1.hour.ago", "--quiet"], dir);
          // "failed" is the word the worker's quiet timer log lets through.
          if (tidied.code !== 0) log(`secondbrain: ${s.label} refreshed, but tidying its checkout failed (${packs} packs) - it is tried again on the next refresh - ${redact(tidied.out, token).slice(0, 160)}`);
        }
      }
