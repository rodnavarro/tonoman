...
/** PURE: a path a Talent may file a page at — inside the brain, a Markdown page, nothing clever. */
export function pagePathTrouble(p: string): string | undefined {
  if (typeof p !== "string" || !p || p.length > 240) return "a page needs a path of reasonable length";
  if (p.startsWith("/") || p.startsWith("\\") || /^[A-Za-z]:/.test(p)) return "a page's path is inside the brain, not from the root of anything";
  const parts = p.split("/");
  if (parts.some((x) => !x || x === "." || x === ".." || x.startsWith(".git"))) return "a page's path has no empty, dotted or git parts";
  if (/[\\\u0000-\u001f]/.test(p)) return "a page's path has no backslashes or control characters";
  if (!/\.md$/i.test(p)) return "a page is a Markdown file";
  return undefined;
}

/** A Talent filing ONE page it wrote, at a path it names (TALENT-FILES-A-PAGE). */
export async function publishPageToBrain(
  store: Pick<BrainStore, "writeFiles">,
  target: { brain: BrainRef; who: string; authorize: () => Promise<boolean>; agentGuid?: string },
  page: { path: string; content: string; note?: string },
): Promise<FilesResult> {
  return store.writeFiles({
    brain: target.brain,
    files: [{ path: page.path, content: page.content }],
    note: (page.note ?? `A page: ${page.path}`).slice(0, 200),
    who: target.who,
    authorize: target.authorize,
    notify: target.agentGuid ? { agentGuid: target.agentGuid, slackUserId: target.who, unattended: true } : undefined,
  });
