...
describe("the harness side", () => {
  it("BRAIN-EVERY-AGENT the MCP server a harness starts is tonoman, one tool, and it answers a brain command", async () => {
    ...
    ask({ id: 3, method: "tools/call", params: { name: "tonoman", arguments: { args: ["brain", "search", "--query", "Jev"] } } });
    expect((await answer(1)).result.serverInfo).toMatchObject({ name: "tonoman" });
    expect(((await answer(2)).result.tools as { name: string }[]).map((t) => t.name)).toEqual(["tonoman"]);
...
describe("a brain whose repository refuses the credential", () => {
  it("BRAIN-REFUSED-IS-NOT-RETRIED the agent is told it is a set-up fault, and nothing goes back to git for that brain while the turn lasts - a later turn tries afresh", async () => {
    let attempts = 0;
    const refusing = createStore({
      root: path.join(tmp, "clones-refused"),
      token: async () => {
        attempts++;
        throw new Error("this brain has no credential to be reached with: there is no credential for where it lives");
      },
      fetchEveryMs: 0,
      log: () => {},
    });
    const b2 = createBroker({
      store: refusing,
      scratch: path.join(tmp, "scratch-refused"),
      log: () => {},
      registry: {
        reach: async (_agent, user): Promise<Reach> => ({ tenant: "test-a", speaker: { accountId: user, name: user, member: true }, brains: [engineering] }),
        recordRepo: async () => {},
      },
    });
    await b2.start();
    try {
      const ask = async (token: string, tool: string, body: Record<string, unknown>) => {
        const r = await fetch(`${b2.url}/brain/${tool}`, { method: "POST", headers: { authorization: `Bearer ${token}`, "content-type": "application/json" }, body: JSON.stringify(body) });
        return { status: r.status, text: await r.text() };
      };
      const { token } = b2.startTurn({ agentGuid: "echo", slackUserId: BEN, who: "Ben" });
      const first = await ask(token, "pages", { brain: "Engineering" });
      expect(first.status).toBe(502);
      expect(first.text).toMatch(/will not help/i);
      expect(first.text).not.toMatch(/just now/i);
      const after = attempts;
      expect(after).toBeGreaterThan(0);
      // Thirteen more tries, as one real turn made: none of them reaches git again.
      for (let i = 0; i < 13; i++) expect((await ask(token, i % 2 ? "pages" : "read", { brain: "Engineering", path: "index.md" })).text).toMatch(/will not help/i);
      expect(attempts).toBe(after);
      // It may have been put right since: the next turn finds out.
      const next = b2.startTurn({ agentGuid: "echo", slackUserId: BEN, who: "Ben" });
      await ask(next.token, "pages", { brain: "Engineering" });
      expect(attempts).toBeGreaterThan(after);
    } finally {
      await b2.close();
    }
  });
