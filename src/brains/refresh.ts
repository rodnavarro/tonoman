...
export interface RefreshDeps {
  ...
  /** Is this brain kept as it is (BRAIN-AS-IS)? Such a brain is never refreshed. */
  keptAsIs?: (brain: BrainRef) => Promise<boolean>;
...
export async function refreshBrain(d: RefreshDeps, brain: BrainRef, name: string): Promise<RefreshOutcome> {
  ...
  // A brain kept as it is is never read by the local model, mapped or published (BRAIN-AS-IS). Asked
  // of the registry too: a refresh queued before the brain was marked so carries an old reference.
  if (brain.asIs || (d.keptAsIs && (await d.keptAsIs(brain).catch(() => true)))) {
    log(`brains: ${brain.id} is kept as it is, so it is not refreshed`);
    return { state: "stale", detail: "kept as it is", again: "never" };
  }
  if (await archived()) {
...
export function createRefreshQueue(o: {
  ...
  return {
    touch(b: BrainRef): void {
      if (b.asIs) return; // BRAIN-AS-IS: nothing to map, nothing queued
    ...
    async resume(): Promise<number> {
      ...
      for (const f of names.filter((x) => x.endsWith(".json"))) {
        ...
        if (b?.asIs) {
          await fs.rm(path.join(o.dir, f), { force: true });
          continue;
        }
        if (b?.id && b.repoUrl) {
          schedule(b, delay);
