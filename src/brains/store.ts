...
export interface BrainRef {
  ...
  name?: string;
  /** How an ADOPTED brain is told from any other brain pointing at the same repository, when its
   *  credential is chosen (BRAIN-REACHED-WITH-ITS-OWN-CREDENTIAL). Not used to find the brain. */
  slug?: string;
  kind?: "personal" | "shared";
  /** Kept exactly as it is (BRAIN-AS-IS): no log.md line, no seed, no map, never refreshed. */
  asIs?: boolean;
}
...
export interface PlanTree {
  ...
  /** Git's name for a file's bytes, or null when it is not there. */
  blobId(path: string): Promise<string | null>;
}

export type PlanResult = { write: { path: string; content: string | Buffer }[]; outcome: unknown } | { nothing: unknown };
...
export interface FilesRequest {
  ...
  basedOn?: string;
  /** Work the files out from the brain as it is on its remote, in EVERY attempt (a receipt's ledger
   *  row, its duplicate check). Replaces `files`. `nothing` writes nothing and is still a success. */
  plan?: (tree: PlanTree) => Promise<PlanResult>;
  /** The commit's first line, exactly (a receipt's `finances: …`). */
  subject?: string;
  /** What recovery needs to work a planned write out again after a restart (serialisable). */
  recover?: Record<string, unknown>;
}

export type FilesResult =
  | { ok: true; paths: string[]; sha: string | null; outcome?: unknown }
...
export function createStore(o: StoreOptions) {
  ...
  async function search(b: BrainRef, query: string, limit = 40): Promise<SearchHit[]> {
    ...
    while (i < out.length && hits.length < limit) {
      ...
      // Long enough to hold a whole ledger row: an edit needs the line's exact words, its end included.
      hits.push({ path: rel, line: Number(out.slice(a + 1, c)), text: out.slice(c + 1, e < 0 ? out.length : e).slice(0, 1200) });
  ...
  /** The brain at its pushed tip, as a plan sees it: read through git, so a link is never followed. */
  function treeAt(dir: string, b: BrainRef): PlanTree {
    const sub = (b.subpath ?? "").replace(/^\/+|\/+$/g, "");
    const strip = (p: string) => (sub && p.startsWith(`${sub}/`) ? p.slice(sub.length + 1) : p);
    return {
      async list(folder) {
        const rel = folder.replace(/^\/+|\/+$/g, "") ? safePagePath(folder, b.subpath) : sub || null;
        const r = await git(["ls-tree", "-r", "-z", remoteRef(b), ...(rel ? ["--", rel] : [])], dir);
        // A folder that is not there lists as nothing, with success. A FAILED listing is not "nothing
        // there": a plan that believed it would file a duplicate, or write over what it could not see.
        if (r.code !== 0) throw new Error(`could not list the brain: ${r.out.slice(0, 200)}`);
        return r.out
          .split("\0")
          .filter(Boolean)
          .map((rec) => {
            const tab = rec.indexOf("\t");
            return { mode: rec.slice(0, 6), p: rec.slice(tab + 1) };
          })
          .filter((e) => e.mode === "100644" || e.mode === "100755")
          .map((e) => strip(e.p));
      },
      async read(p) {
        const rel = safePagePath(p, b.subpath);
        if (!rel) return null;
        const blob = await blobOf(dir, remoteRef(b), rel);
        if (!blob || !(await plainPath(dir, remoteRef(b), rel))) return null;
        // A read that failed or was cut short is not the file: appending to half a ledger and pushing
        // it would replace the whole one.
        const r = await git(["cat-file", "blob", blob], dir);
        if (r.code !== 0) throw new Error(`could not read "${p}" from the brain`);
        return r.out;
      },
      async blobId(p) {
        const rel = safePagePath(p, b.subpath);
        return rel ? blobOf(dir, remoteRef(b), rel) : null;
      },
    };
  }

  async function attemptFiles(req: FilesRequest, rels0: string[], opId: string): Promise<FilesResult | "retry"> {
    const b = req.brain;
    const token = await o.token(b);
    const dir = await fetchNow(b);
    ...
    let outcome: unknown;
    if (req.plan) {
      const planned = await req.plan(treeAt(dir, b));
      if ("nothing" in planned) return { ok: true, paths: [], sha: null, outcome: planned.nothing };
      outcome = planned.outcome;
      files = planned.write;
      rels = [];
      for (const f of files) {
        const rel = safePagePath(f.path, b.subpath);
        const bare = safePagePath(f.path);
        if (!rel || !bare || bare.toLowerCase() === "log.md" || isRefreshPath(bare)) return { ok: false, reason: "bad-path", detail: `"${f.path}" is not a name that can be written` };
        rels.push(rel);
      }
    }
    if (req.basedOn) {
    ...
    const logRel = safePagePath("log.md", b.subpath)!;
    // A brain kept as it is gets no log.md line (BRAIN-AS-IS).
    const logs = !req.system && !b.asIs;
    ...
    for (const rel of ownRel ? (sub ? [sub] : []) : [...rels, ...(logs ? [logRel] : [])]) {
    ...
    try {
      ...
      for (let i = 0; i < rels.length; i++) {
        ...
        await fs.writeFile(target, files[i]!.content);
      }
      if (req.plan) {
        // A planned filing is staged as the exact bytes it was given, by git's own name for them: the
        // repo's ignore rules cannot leave the file out (a ledger row with no evidence, said to be
        // "filed"), and its text rules cannot rewrite a photo's bytes.
        for (let i = 0; i < rels.length; i++) {
          const h = await git(["hash-object", "-w", "--no-filters", "--", rels[i]], wt);
          const id = h.out.trim();
          if (h.code !== 0 || !/^[0-9a-f]{40,64}$/.test(id)) return { ok: false, reason: "push-failed", detail: `could not store "${files[i]!.path}"` };
          const staged = await git(["update-index", "--add", "--cacheinfo", `100644,${id},${rels[i]}`], wt);
          if (staged.code !== 0) return { ok: false, reason: "push-failed", detail: `could not stage "${files[i]!.path}": ${staged.out.slice(0, 200)}` };
        }
      } else {
        const added = await git(["add", "-A", "--", ...rels, ...(ownRel ? [ownRel] : [])], wt);
        if (added.code !== 0) return { ok: false, reason: "push-failed", detail: `could not stage the change: ${added.out.slice(0, 200)}` };
      }
      // Nothing changed (a re-run of something already filed): no commit, no log line, still a success.
      if ((await git(["diff", "--cached", "--quiet"], wt)).code === 0) return { ok: true, paths: files.map((f) => f.path), sha: null, outcome };
      if (logs) {
        const logFile = path.join(wt, logRel);
        const prior = await fs.readFile(logFile, "utf8").catch(() => "# Log\n\n");
        await fs.mkdir(path.dirname(logFile), { recursive: true });
      ...
      await git(["update-ref", remoteRef(b), sha], dir);
      lastFetch.set(b.id, Date.now());
      return { ok: true, paths: files.map((f) => f.path), sha, outcome };
    } finally {
      await git(["worktree", "remove", "--force", wt], dir);
      await fs.rm(wt, { recursive: true, force: true });
    }
  ...
  async function writeFiles(req: FilesRequest): Promise<FilesResult> {
    const rels: string[] = [];
    for (const f of req.plan ? [] : req.files) {
    ...
    if (!rels.length && !req.plan) return { ok: false, reason: "bad-path", detail: "nothing to write" };
    const opId = randomUUID();
    await journal(
      opId,
      req.plan
        ? { status: "pending", kind: "planned", brain: req.brain, recover: req.recover ?? null, note: req.note, who: req.who, notify: req.notify ?? null }
        : { status: "pending", kind: "files", system: !!req.system, brain: req.brain, files: req.files, note: req.note, who: req.who, notify: req.notify ?? null },
    );
    return serial(req.brain, async () => {
      for (let i = 0; i < 4; i++) {
        ...
        if (r.ok && r.sha && !req.system && !req.brain.asIs) o.onPushed?.(req.brain);
        return r;
      }
      await journal(opId, { status: "failed", detail: "the brain kept changing under the write" });
